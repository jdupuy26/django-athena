...
class Repo(models.Model):
    ...
    name = models.CharField(max_length=100, help_text="Enter the name for the repo", default="")

    url = models.URLField(max_length=500, help_text="Enter the url for the GH repo", default="")
...
class Fork(models.Model):
    ...
    name = models.CharField(max_length=100, help_text="Enter the name for the fork", default="")

    url = models.URLField(max_length=500, help_text="Enter the url for the GH fork", default="")
    ...
    code = models.OneToOneField(
        "Code", help_text="Select a code for this fork", on_delete=models.SET_NULL, null=True
    )

    # Here foreign key is used because a fork has one repo, but the repo can have many forks
    repo = models.ForeignKey(
        "Repo", help_text="Select a repo for this fork", on_delete=models.SET_NULL, null=True
    )
...
class Branch(models.Model):
    ...
    name = models.CharField(max_length=100, help_text="Enter the name for the branch", default="")

    # here foreign key is used because a branch has one fork, but a fork can have many branches
    fork = models.ForeignKey(
        "Fork", help_text="Select a fork for this branch", on_delete=models.SET_NULL, null=True
    )

    # Here foreign key is used because a fork has one repo, but the repo can have many forks
    repo = models.ForeignKey(
        "Repo", help_text="Select a repo for this branch", on_delete=models.SET_NULL, null=True
    )
...
class Code(models.Model):
    ...
    path = models.CharField(
        default=settings.BASE_DIR + "/mycode",
    ...
    def compile(self, options="-j", **filter):
        ...
        try:
            ...
            result = subprocess.check_output(
                command, stderr=subprocess.STDOUT, cwd=self.path, universal_newlines=True
            )
            logger.info("Compilation details: {}".format(result))
        except subprocess.CalledProcessError as e:
            logger.exception("{}/make hit an exception...".format(self.name))
    ...
    def run(self, options="-i", **filter):
        ...
        command = ["./athena", options, self.configuration.input_file]
        try:
            logger.info("Running athena with command {}".format(command))
            result = subprocess.check_output(
                command, stderr=subprocess.STDOUT, cwd=self.path + "/bin/", universal_newlines=True
            )
            logger.info("Run details: {}".format(result))
        except subprocess.CalledProcessError as e:
            logger.exception("{}/bin/athena hit an exception...".format(self.name))
    ...
    def list_input_files(self):
        ...
        return [
            os.path.join(dp, f)
            for dp, dn, fn in os.walk(os.path.expanduser(self.path + "/inputs"))
            for f in fn
        ]
...
class Configuration(models.Model):
    ...
    prob = models.CharField(
        help_text="Select a problem to configure", max_length=100, default="blast"
    )
    ...
    fluxcl = models.CharField(
        help_text="Select Riemann solver for the collisionless variables",
        choices=(("hlle", "Hartan, Lax, van Leer, Einfeldt (HLLE)"), ("roe", "Roe Solver")),
        default="roe",
        max_length=100,
    )

    grav = models.CharField(
    ...
    nscalars = models.IntegerField(help_text="Enter number of scalars", default=0)

    nghost = models.IntegerField(help_text="Enter the number of ghost zones", default=2)

    bfield = models.BooleanField(help_text="Enable magnetic fields?", default=False)

    cless = models.BooleanField(help_text="Enable collisionless solver?", default=False)

    cless_only = models.BooleanField(help_text="Enable collisionless-only mode?", default=False)

    spec_rel = models.BooleanField(help_text="Enable special relativity?", default=False)

    gen_rel = models.BooleanField(help_text="Enable general relativity?", default=False)

    transforms = models.BooleanField(
        help_text="Enable interface frame transformations for GR?", default=False
    )

    shear = models.BooleanField(help_text="Enable shearing box?", default=False)

    dual_energy = models.BooleanField(help_text="Enable dual energy?", default=False)

    debug = models.BooleanField(help_text="Enable debug mode?", default=False)

    float_precision = models.BooleanField(help_text="Enable single precision?", default=False)

    openmp = models.BooleanField(help_text="Enable parallelization with OpenMP?", default=False)

    mpi = models.BooleanField(help_text="Enable parallelization with MPI?", default=False)

    fft = models.BooleanField(help_text="Enable fast fourier transform?", default=False)

    fftw_path = models.CharField(
        help_text="Enter path to FFTW libraries", default="", max_length=100, blank=True
    )

    hdf5 = models.BooleanField(help_text="Enable HDF5 Output", default=False)

    hdf5_path = models.CharField(
        help_text="Enter path to HDF5 libraries", default="", max_length=100, blank=True
    )

    cxx = models.CharField(
    ...
    include = models.CharField(
        help_text="use -Ipath when compiling", default="", max_length=500, blank=True
    )

    lib = models.CharField(
        help_text="use -Lpath when linking", default="", max_length=500, blank=True
    )
    ...
    def configure(self):
        ...
        try:
            result = subprocess.check_output(
                command, stderr=subprocess.STDOUT, cwd=self.code.path, universal_newlines=True
            )
            logger.info("Configuration details: {}".format(result))
        except subprocess.CalledProcessError as e:
            logger.exception("{}/configure.py hit an exception...".format(self.code.name))
